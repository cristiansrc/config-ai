...
- If verdict is `not ready`:
  1. Set active_spec status to `draft` or `planning`.
...
- For every non-trivial SDD workflow, expect a temporary shared context file inside the active repository at `docs/specs/.working/<increment-name>-sdd-context.md`.
- **Placeholder Guard**: Replace `<increment-name>` with the actual feature name.
...
- Do not return `ready` if a task board status other than `todo`, `in_progress`, `done`, or `blocked`.
