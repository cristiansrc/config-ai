...
- Cambio operativo vigente: Spec Remediator debe invocar validaciones solo mediante `spec-validator` con `opencode-go/gpt-luna`; cualquier validacion ejecutada con otro modelo debe bloquearse como `Blocked: wrong validator model`.
...
| architect-executor    | opencode-go/deepseek-v4-pro   | allow             | allow | Implementa tareas complejas y arquitectura local con razonamiento denso cuando NO existe especificación SDD completa.                               |
...
| database-architect    | opencode-go/deepseek-v4-flash | allow             | allow | Diseña esquemas DB relacionales, migraciones Flyway/Liquibase e índices sin tiempo de inactividad (Zero-Downtime DB Migrations).                   |
...
| enterprise-architect  | opencode-go/deepseek-v4-pro   | allow             | deny  | Define el System Landscape, fronteras de microservicios y flujos globales siguiendo `enterprise-architecture-standard`.                           |
...
| executor              | opencode-go/deepseek-v4-flash | allow             | allow | Implementa código verificado (Temp 0.15) cuando EXISTE especificación SDD validada siguiendo los estándares del stack correspondiente.              |
...
| functional-tester-agent| opencode-go/deepseek-v4-flash| allow             | allow | Diseña plan, ejecuta y valida pruebas funcionales y UI/E2E en frontends con Puppeteer MCP.                                                         |
...
| solution-architect    | opencode-go/deepseek-v4-pro   | allow             | deny  | Elige patrones de diseño siguiendo `design-patterns-standard`. Colabora con `enterprise-architect` para alinear el diseño local con el global.    |
...
| test-architect        | opencode-go/deepseek-v4-flash | allow             | allow | Diseña la estrategia de pruebas y genera casos de test siguiendo `testing-strategy`.                                                              |
...
| hyprmind-deep-thinker  | opencode-go/deepseek-v4-pro   | deny              | deny  | Filósofo y motor de razonamiento denso para discusiones complejas o diseño abstracto.                                                              |
