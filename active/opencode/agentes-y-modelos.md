...
|-----------------------|-------------------------------|-------------------|-------|---------------------------------------------------------------------------------------------------------------------------------------------------|
| architect-executor    | opencode/deepseek-v4-flash-free | allow             | allow | Implementa tareas de arquitectura local y lógica compleja siguiendo `hexagonal-architecture`.                                                     |
| devops-architect      | opencode/north-mini-code-free | allow             | allow | Especialista en Infraestructura como Código y CI/CD siguiendo `docker-standard` y `observability-standard`.                                       |
...
| documentation         | opencode/north-mini-code-free  | allow             | deny  | Gestiona el ciclo de vida de la documentación siguiendo `documentation-lifecycle` y `documentation-standards`.                                    |
...
| enterprise-architect  | google/gemini-3.1-pro-preview | allow             | deny  | Define el System Landscape, fronteras de microservicios y flujos globales siguiendo `enterprise-architecture-standard`.                           |
...
| spec-remediator       | opencode/north-mini-code-free | allow             | deny  | Corrige hallazgos de validación de forma iterativa siguiendo `spec-remediation`.                                                                  |
...
| functional-test-planner| opencode/north-mini-code-free      | allow             | deny  | Analiza las specs a nivel de workspace/proyectos y diseña planes de pruebas funcionales y flujos de usuario estructurados siguiendo `functional-testing-standard`. |
| functional-tester-agent| opencode/deepseek-v4-flash-free       | allow             | allow | Diseña, ejecuta y valida pruebas funcionales y UI/E2E en frontends. Automatiza la detección, reporte y corrección mecánica de errores siguiendo `functional-testing-standard`. |
| git-executor          | opencode/north-mini-code-free | allow             | allow | Centraliza de manera exclusiva todas las interacciones del repositorio con Git (ramas, commits, checkout, pushes).                                |
...
| master-orchestrator   | opencode/deepseek-v4-flash-free | deny              | allow | Agente Maestro / Orquestador Contextual. Mantiene el contexto de la spec y delega tareas a subagentes de forma estructurada.                      |
