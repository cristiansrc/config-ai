...
- Cambio operativo vigente (temporal): Spec Remediator debe invocar validaciones solo mediante `spec-validator` con `opencode-go/qwen3.7-plus`; cualquier validacion ejecutada con otro modelo debe bloquearse como `Blocked: wrong validator model`.
...
|-----------------------|-------------------------------|-------------------|-------|---------------------------------------------------------------------------------------------------------------------------------------------------|
| architect-executor    | opencode-go/deepseek-v4-flash | allow             | allow | Implementa tareas de arquitectura local y lógica compleja siguiendo `hexagonal-architecture`.                                                     |
| devops-architect      | opencode-go/deepseek-v4-flash | allow             | allow | Especialista en Infraestructura como Código y CI/CD siguiendo `docker-standard` y `observability-standard`.                                       |
| context-curator       | opencode-go/qwen3.7-plus    | deny              | deny  | Gestiona la ventana de contexto y reduce ruido siguiendo `context-curation` y `context-pinning`.                                                  |
| documentation         | opencode-go/deepseek-v4-flash  | allow             | deny  | Gestiona el ciclo de vida de la documentación siguiendo `documentation-lifecycle` y `documentation-standards`.                                    |
...
| final-validation      | opencode-go/qwen3.7-plus | deny              | allow | Garantiza la calidad final y cobertura mínima siguiendo `testing-strategy` y `pre-flight-check`.                                                  |
| enterprise-architect  | opencode-go/qwen3.7-plus      | allow             | deny  | Define el System Landscape, fronteras de microservicios y flujos globales siguiendo `enterprise-architecture-standard`.                           |
| enterprise-spec-validator| opencode-go/qwen3.7-plus     | allow             | deny  | Valida la coherencia global del Solution Workspace, contratos de integración y deuda técnica global siguiendo `workspace-coordination`.          |
| solution-architect    | opencode-go/qwen3.7-plus    | allow             | deny  | Elige patrones de diseño siguiendo `design-patterns-standard`. Colabora con `enterprise-architect` para alinear el diseño local con el global.    |
| planner               | opencode-go/qwen3.7-plus      | allow             | deny  | Planifica incrementos SDD y diseña contratos. Consulta a `solution-architect` para decisiones de patrones complejos.                              |
| refactor              | opencode-go/deepseek-v4-pro         | allow             | allow | Refactoriza código existente siguiendo `refactor-patterns` y `refactor-hexagonal-bridge`.                                                         |
| requirements-analyst  | opencode-go/qwen3.7-plus    | allow             | deny  | Realiza el levantamiento de requerimientos funcionales siguiendo `requirements-gathering`.                                                        |
| reviewer              | opencode-go/qwen3.7-plus      | deny              | allow | Audita el código generado buscando drift y bugs siguiendo `code-review-checklist`.                                                                |
| security-reviewer     | opencode-go/qwen3.7-plus    | deny              | allow | Valida la postura de seguridad siguiendo `security-standards` y `keycloak-standard`.                                                              |
...
| spec-validator        | opencode-go/qwen3.7-plus    | allow limitado    | deny  | Valida la consistencia de los artefactos SDD siguiendo `spec-driven-development`.                                                                 |
| task-decomposer       | opencode-go/deepseek-v4-pro    | allow             | deny  | Descommpone especificaciones en tareas atómicas siguiendo los contratos de `spec-driven-development`.                                              |
| test-architect        | opencode-go/deepseek-v4-flash | allow             | allow | Diseña la estrategia de pruebas y genera casas de test siguiendo `testing-strategy`.                                                              |
| functional-test-planner| opencode-go/deepseek-v4-flash      | allow             | deny  | Analiza las specs a nivel de workspace/proyectos y diseña planes de pruebas funcionales y flujos de usuario estructurados siguiendo `functional-testing-standard`. |
| functional-tester-agent| opencode-go/deepseek-v4-flash       | allow             | allow | Diseña, ejecuta y valida pruebas funcionales y UI/E2E en frontends. Automatiza la detección, reporte y corrección mecánica de errores siguiendo `functional-testing-standard`. |
| git-executor          | opencode-go/deepseek-v4-flash | allow             | allow | Centraliza de manera exclusiva todas las interacciones del repositorio con Git (ramas, commits, checkout, pushes).                                |
| hyprmind-orchestrator  | opencode-go/qwen3.7-plus    | allow             | allow | (V.I.E.R.N.E.S.) Orquestador conversacional principal de tu sistema y manejador del workspace. |
| hyprmind-vision-analyst| opencode-go/deepseek-v4-flash       | deny              | deny  | El "Ojo Biónico" que procesa y explica tus capturas de pantalla o interfaces visuales.                                                                             |
...
| master-orchestrator   | opencode-go/deepseek-v4-flash | deny              | allow | Agente Maestro / Orquestador Contextual. Mantiene el contexto de la spec y delega tareas a subagentes de forma estructurada.                      |
