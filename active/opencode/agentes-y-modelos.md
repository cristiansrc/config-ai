...
| context-curator       | opencode/deepseek-v4-flash-free | deny              | deny  | Gestiona la ventana de contexto y reduce ruido siguiendo `context-curation` y `context-pinning`.                                                  |
...
| final-validation      | opencode/deepseek-v4-flash-free | deny              | allow | Garantiza la calidad final y cobertura mínima siguiendo `testing-strategy` y `pre-flight-check`.                                                  |
| enterprise-architect  | opencode/deepseek-v4-flash-free | allow             | deny  | Define el System Landscape, fronteras de microservicios y flujos globales siguiendo `enterprise-architecture-standard`.                           |
| enterprise-spec-validator| opencode/deepseek-v4-flash-free | allow             | deny  | Valida la coherencia global del Solution Workspace, contratos de integración y deuda técnica global siguiendo `workspace-coordination`.          |
| solution-architect    | opencode/deepseek-v4-flash-free | allow             | deny  | Elige patrones de diseño siguiendo `design-patterns-standard`. Colabora con `enterprise-architect` para alinear el diseño local con el global.    |
| planner               | opencode/deepseek-v4-flash-free | allow             | deny  | Planifica incrementos SDD y diseña contratos. Consulta a `solution-architect` para decisiones de patrones complejos.                              |
| refactor              | opencode/deepseek-v4-flash-free | allow             | allow | Refactoriza código existente siguiendo `refactor-patterns` y `refactor-hexagonal-bridge`.                                                         |
| requirements-analyst  | opencode/deepseek-v4-flash-free | allow             | deny  | Realiza el levantamiento de requerimientos funcionales siguiendo `requirements-gathering`.                                                        |
| reviewer              | opencode/deepseek-v4-flash-free | deny              | allow | Audita el código generado buscando drift y bugs siguiendo `code-review-checklist`.                                                                |
| security-reviewer     | opencode/deepseek-v4-flash-free | deny              | allow | Valida la postura de seguridad siguiendo `security-standards` y `keycloak-standard`.                                                              |
...
| spec-validator        | opencode/deepseek-v4-flash-free | allow limitado    | deny  | Valida la consistencia de los artefactos SDD siguiendo `spec-driven-development`.                                                                 |
| task-decomposer       | opencode/deepseek-v4-flash-free | allow             | deny  | Descompone especificaciones en tareas atómicas siguiendo los contratos de `spec-driven-development`.                                              |
