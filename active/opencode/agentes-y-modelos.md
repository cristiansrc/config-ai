...
| architect-executor    | opencode/deepseek-v4-pro      | allow             | allow | Implementa tareas complejas y arquitectura local con razonamiento denso cuando NO existe especificación SDD completa.                               |
...
| database-architect    | opencode/deepseek-v4-flash    | allow             | allow | Diseña esquemas DB relacionales, migraciones Flyway/Liquibase e índices sin tiempo de inactividad (Zero-Downtime DB Migrations).                   |
| devops-architect      | opencode/deepseek-v4-flash    | allow             | allow | Especialista en Infraestructura como Código y CI/CD siguiendo `docker-standard` y `observability-standard`.                                       |
| documentation         | opencode/deepseek-v4-flash    | allow             | deny  | Gestiona el ciclo de vida de la documentación siguiendo `documentation-lifecycle` y `documentation-standards`.                                    |
| enterprise-architect  | opencode/deepseek-v4-pro      | allow             | deny  | Define el System Landscape, fronteras de microservicios y flujos globales siguiendo `enterprise-architecture-standard`.                           |
...
| executor              | opencode/deepseek-v4-flash    | allow             | allow | Implementa código verificado (Temp 0.15) cuando EXISTE especificación SDD validada siguiendo los estándares del stack correspondiente.              |
...
| functional-tester-agent| opencode/deepseek-v4-flash   | allow             | allow | Diseña plan, ejecuta y valida pruebas funcionales y UI/E2E en frontends con Puppeteer MCP.                                                         |
| git-executor          | opencode/deepseek-v4-flash    | allow             | allow | Centraliza de manera exclusiva todas las interacciones del repositorio con Git (ramas, commits, checkout, pushes).                                |
...
| solution-architect    | opencode/deepseek-v4-pro      | allow             | deny  | Elige patrones de diseño siguiendo `design-patterns-standard`. Colabora con `enterprise-architect` para alinear el diseño local con el global.    |
| spec-remediator       | opencode/deepseek-v4-flash    | allow             | deny  | Corrige hallazgos de validación de forma iterativa siguiendo `spec-remediation`.                                                                  |
...
| test-architect        | opencode/deepseek-v4-flash    | allow             | allow | Diseña la estrategia de pruebas y genera casos de test siguiendo `testing-strategy`.                                                              |
...
| hyprmind-vision-analyst| opencode/deepseek-v4-flash   | deny              | deny  | El "Ojo Biónico" que procesa y explica tus capturas de pantalla o interfaces visuales.                                                             |
