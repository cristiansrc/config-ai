...
- `trace_id`: correlación desde MDC (en **`snake_case`** para coincidir con `restful-standard`).
- `details`: lista opcional para errores de campo (usar **`snake_case`** para atributos).

## 4. RestControllerAdvice e Integración Hexagonal

Implementar un único advice global en la capa de **Infraestructura**:
