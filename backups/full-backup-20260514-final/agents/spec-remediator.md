...
- Before finishing, you MUST verify that Spec Validator has applied "The Three-Point Update" if the verdict is `ready`:
  1. Primary spec status is `validated-not-executed`.
  2. Shared context `Current status` is `validated-not-executed`.
  3. Shared context contains `## Spec Validator Approval` with `verdict: ready`.
- If Spec Validator provided a `ready` verdict but failed to update these 3 points, Spec Remediator must apply the updates itself or request the update from Spec Validator before finishing.

Required workflow:
1. Rehydrate from disk.
2. Call or request the `spec-validator` agent first for a full validation of all active specs and canonical artifacts. This validation must be performed by the dedicated `spec-validator` agent configured with `model: lmstudio/qwen/qwen3.6-35b-a3b`.
...
- The only valid validation agent is `spec-validator` with `model: lmstudio/qwen/qwen3.6-35b-a3b`.
...
- If the tool UI, runtime log, or launched subagent name shows `general`, cancel the validation immediately and report `Blocked: wrong agent route - use spec-validator with lmstudio/qwen/qwen3.6-35b-a3b`.
...
- If the orchestration system starts validation with any non-`lmstudio/qwen/qwen3.6-35b-a3b` model, stop and report `Blocked: wrong validator model`.
- Treat any validation response as invalid unless it explicitly comes from `spec-validator`, `spec-validation`, or `spect-validation`, and the runtime clearly shows the invoked agent is configured with `lmstudio/qwen/qwen3.6-35b-a3b`.
- When requesting validation, name the target agent explicitly as `spec-validator` and include: `Use the spec-validator agent configured with lmstudio/qwen/qwen3.6-35b-a3b.`
